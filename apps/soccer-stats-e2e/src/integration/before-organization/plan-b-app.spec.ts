...
describe('The soccer stats app player page ', () => {
  const testPlayerName = `e2e-player-${now}`;
...
describe('The soccer stats app game management page', () => {
  ...
  it('should start with no players, shots, or cards on new games', () => {
    const testGameName = `e2e-test-game-${now}`;
    cy.visit(`/games`);
  ...
  describe('player and game management', () => {
    beforeEach(() => {
      createPlayerOnServer(`e2e-player-1-${now}`)
        .its('body')
        .as('player1');
      createPlayerOnServer(`e2e-player-2-${now}`)
        .its('body')
        .as('player2');
      createGameOnServer(`e2e-game-${now}`).its('body').as('game');
    });

    it('should allow game deletion', function () {
      cy.visit(`/games/${this.game.id}`);
      deleteCurrentGame();
      cy.contains(this.game.name).should('not.exist');
    });
